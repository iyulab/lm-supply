...
/// <summary>
/// A translator loaded with <see cref="ExecutionProvider.Cpu"/> does not probe the GPU. The download ranking once did (it
...
{
    private const string Alias = "ko-en";
    ...
    public async Task CpuProvider_PresetAlias_LoadsNoNvidiaDriverLibrary()
    {
        ...
        try
        {
            await using var model = await LocalTranslator.LoadAsync(Alias,
                new TranslatorOptions { Provider = ExecutionProvider.Cpu },
    ...
    public async Task AutoProvider_PresetAlias_OnAnNvidiaHost_ProbesTheDriver()
    {
        ...
        await using var model = await LocalTranslator.LoadAsync(Alias,
            new TranslatorOptions { Provider = ExecutionProvider.Auto },
            cancellationToken: TestContext.Current.CancellationToken);
