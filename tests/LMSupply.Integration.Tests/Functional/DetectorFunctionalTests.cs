...
public class DetectorFunctionalTests
{
    ...
    [Fact]
    [Trait("Axis", "Quality")]
    public async Task Face_FindsNothingInStructurelessInput()
    {
        // This assertion is weak on its own - an empty result is also what a decoder wired to the wrong
        // strides returns - so it earns its place only next to the recorded-anchor tests in
        // LMSupply.Detector.Tests, which pin the boxes a real run actually produced.
        //
        // It is here for a second reason. Before the resolver was fixed to fetch the file the model info
        // names, this alias silently loaded an int8 build of the same weights, and that build fired on this
        // gradient at 0.271 - just over the default threshold, with a box covering most of the frame. The
        // quantization noise looked exactly like a real detection. Whatever else this test does, it fails
        // again if a substituted model comes back.
        await using var model = await LocalDetector.LoadAsync("face", cancellationToken: TestContext.Current.CancellationToken);
