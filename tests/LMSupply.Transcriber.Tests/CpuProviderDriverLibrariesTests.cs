...
/// <summary>
/// A transcriber loaded with <see cref="ExecutionProvider.Cpu"/> does not probe the GPU. The download ranking once did (it
...
public sealed class CpuProviderDriverLibrariesTests
{
    ...
    public async Task CpuProvider_PresetAlias_LoadsNoNvidiaDriverLibrary()
    {
        ...
        try
        {
            await using var model = await LocalTranscriber.LoadAsync(Alias,
                new TranscriberOptions { Provider = ExecutionProvider.Cpu },
    ...
    public async Task AutoProvider_PresetAlias_OnAnNvidiaHost_ProbesTheDriver()
    {
        ...
        await using var model = await LocalTranscriber.LoadAsync(Alias,
            new TranscriberOptions { Provider = ExecutionProvider.Auto },
            cancellationToken: TestContext.Current.CancellationToken);
